...
#flags.DEFINE_string('model', 'vgg', 'Model size')
flags.DEFINE_string('output_dir', './model_natural', 'Output directory.')

flags.DEFINE_float('width_num', 2, 'Width Number for CNN')
# Options.
flags.DEFINE_integer('steps', 20001, 'Number of steps in total.')
flags.DEFINE_integer('test_every_n', 500,
                     'Number of steps between testing iterations.')
flags.DEFINE_integer('warmup_steps', 500, 'Number of warm-up steps.')
flags.DEFINE_integer('rampup_steps', 6500, 'Number of ramp-up steps.')
flags.DEFINE_integer('batch_size', 20, 'Batch size.')
flags.DEFINE_float('epsilon', 8/255, 'Target epsilon.')
flags.DEFINE_string('learning_rate', '1e-4,1e-5@10000,1e-6@15000',
...
def layers(width_num=1):
    """Returns the layer specification for a given model name."""

    return (
        ('conv2d', (3, 3), int(4*width_num), 'SAME', 1),
        ('activation', 'relu'),
        ('conv2d', (3, 3), int(4*width_num), 'SAME', 2),
        ('activation', 'relu'),
        ('conv2d', (3, 3), int(8*width_num), 'SAME', 1),
        ('activation', 'relu'),
        ('conv2d', (3, 3), int(8*width_num), 'SAME', 2),
        ('activation', 'relu'),
        ('conv2d', (3, 3), int(16*width_num), 'SAME', 1),
        ('activation', 'relu'),
        ('conv2d', (3, 3), int(16*width_num), 'SAME', 2),
        ('activation', 'relu'),
        ('conv2d', (3, 3), int(32*width_num), 'SAME', 1),
        ('activation', 'relu'),
        ('conv2d', (3, 3), int(32*width_num), 'SAME', 2),
        ('activation', 'relu'),
        ('linear', 200),
        ('activation', 'relu'))
...
def main(unused_args):
    ...
    print(train_data.shape, test_data.shape, train_label.shape, test_label.shape, np.amax(train_data), np.amin(test_data))
